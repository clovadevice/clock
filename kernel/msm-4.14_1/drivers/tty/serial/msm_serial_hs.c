	...
	*msm_hs_dt_to_pdata(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(node, "qcom,set-autosuspend-delay",
				&pdata->autosuspend_delay);
	pr_err("DTS qcom,set_autosuspend_delay %d",pdata->autosuspend_delay);
...
#ifdef CONFIG_PM
static void  msm_serial_hs_rt_init(struct uart_port *uport,unsigned int suspend_delay)
{
	struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);
	...
	if(suspend_delay > 0)
		pm_runtime_set_autosuspend_delay(uport->dev, suspend_delay);
	else
...
static int msm_hs_probe(struct platform_device *pdev)
{
	...
	if (!ret) {
		msm_hs_clk_bus_unvote(msm_uport);
		msm_serial_hs_rt_init(uport,pdata->autosuspend_delay);
		return ret;
	}
